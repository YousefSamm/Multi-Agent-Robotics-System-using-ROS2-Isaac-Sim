...
def setup_simulation():
    ...
    stage_path = os.path.join(
        script_dir, 
        "Collected_noham_sim_mobile_manipulator", 
        "noham_sim_mobile_manipulator.usd"
    )
        ...
        if stage:
            for prim in stage.Traverse():
                if "Graph" in str(prim.GetPath()):
                    try:
                        graph = og.get_graph_by_path(str(prim.GetPath()))
                        if graph and hasattr(graph, 'is_valid') and graph.is_valid():
                            if hasattr(graph, 'is_running') and not graph.is_running():
